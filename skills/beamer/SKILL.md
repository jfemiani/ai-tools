...
Repository-specific note: in this repo, `SKILL.md` is the canonical source for defaults and rules. `AGENTS.md` is a Codex-facing execution wrapper and should not redefine policy.
...
- **Default presenter**: `\author{CSE 534}` and `\institute{Miami University}`. Override only if user specifies otherwise.
